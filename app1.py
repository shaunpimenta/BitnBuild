...
def retrieve(id):
    ...
    cur = connection.cursor()
    query = f"select * from students1 where ID='{str(id)}';"
    print(query)
    cur.execute(query)
...
def plotgpa(id):
    try:
        ...
        #    'CGPA expected']
        cur=connection.cursor()
        query=f"select CGPAinlastsem from students1 where id='{id}';"
        cur.execute(query)
        test = cur.fetchall()

        # connection.commit()
        connection.close()
        return list(test[0])[0]

    except Exception as e:
        print(e)
def updater(id,att,gpa,rns,work):
    try:
        connection=sqlite3.connect('studentdb.db')
        index=['Working', 'Romantic relationship', 'Salary', 'Transpotationmode',
           'Siblings', 'Reading(non-scientific)', 'Seminars/Conference',
           ...
           'CGPA expected']
        cur=connection.cursor()
        query=f"update students1 set Working={work}, Attendance={att},Rns={rns},CGPAinlastsem={gpa} where id='{id}';"
        cur.execute(query)
        connection.commit()
        connection.close()
    except Exception as e:
...
def update():
    if request.method=="POST":
        ...
        rns = request.form.get("rns")
        gpa = request.form.get("gpa")
        work = request.form.get("work")
        print(att,gpa,rns,work,id)
        updater(id, att, gpa, rns, work)
        # print(a)
...
def plot():
    if request.method=="POST":
        ...
        s1=request.form.get('sem1')
        s2 = request.form.get('sem2')
        s3 = request.form.get('sem3')
        s4=plotgpa(id)
        import matplotlib.pyplot as plt
        import numpy as np
        y = np.array([s1, s2, s3, s4])  # X-axis points
        x = np.array([2020, 2021, 2022, 2023])  # Y-axis points
